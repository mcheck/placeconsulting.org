...
function SelectPage()
{
	var spage = GetE('mason-pages-list');
	
	if( spage.value != '-' )
	{
		GetE('txtUrl').value = spage.value;
		GetE('cmbLinkProtocol').options[4].selected = true;
	}
}

function LoadPages()
{
	var spage = GetE('mason-pages-layer');
	spage.innerHTML = oEditor.parent.listPages;
	OnUrlChange();
}
